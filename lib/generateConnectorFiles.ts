...
//
// This does NOT depend on chart/table mappings at all — it's the same
// generic file for every connector except for the pre-export line, so it
// can be generated as soon as a connector ID and (optional) pre-export
// function name are known, before any Notion page has even been looked at.
...
export function generateMappingRows(charts: ChartMapping[], tableRows: TableRowMapping[]): string[][] {
  ...
  if (tableRows.length > 0) {
    rows.push(['Tables']);
    const maxCols = Math.max(...tableRows.map(r => r.sourceCells.length));
  ...
  if (charts.length > 0) {
    rows.push(['Charts']);
    rows.push(['Block ID', '', 'Sheet tab', 'Chart title']);
    for (const chart of charts) {
      rows.push(['Block ID', chart.blockId, chart.tabName, chart.chartTitle]);
    }
  }
