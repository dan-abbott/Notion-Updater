...
**Details:**
- `lib/github.ts` generalized from a `connectors.json`-specific client into generic `fetchJsonFile()`/`commitJsonFile()` helpers, with `fetchConnectorsFile()`/`commitConnectorsFile()` and new `fetchMappingFile()`/`commitMappingFile()` built on top of the same primitives. `fetchJsonFile()` treats a 404 as "doesn't exist yet" (returns an empty default + `sha: null`) rather than throwing — the normal case the first time a connector goes through the wizard.
- New `app/api/setup/mapping/route.ts`: `GET ?connectorId=` returns that connector's current mapping (or an empty one if it's never been saved); `POST` commits a FULL replacement — same fresh-sha-immediately-before-write pattern as the connectors.json admin endpoints, for the same optimistic-concurrency protection. **Not** behind admin auth (consistent with the rest of `/setup`, which has never been gated) — this is a lower-stakes action than registering a connector (`/api/admin/connectors`), since it only affects a Sheet's own mapping layout, not live routing.
- `generateMappingRows()` now always prepends `['⚠️ DO NOT EDIT THIS TAB MANUALLY']` and `['To make changes, use:', 'https://notion-updater-pi.vercel.app/admin']` (plus the existing Connector ID row) — all still safely ignored by `readMappingSheet()`, which only ever recognizes exact `"Row Block ID"`/`"Block ID"` labels in column A.
- Wizard's "Edit mapping" flow now actually loads state: `loadExistingMapping()` fetches the connector's saved mapping JSON and pre-fills `chartDetails`/`tableRowDetails` for every entry whose block ID still exists on the page (via `collectBlockInfo()`, which recurses into `columns` items) — entries whose block was deleted from Notion are silently dropped rather than kept as orphaned state. Table row cell counts are resized to the table's CURRENT column count, not whatever the saved mapping assumed.
- `handleGenerateMapping()` now does two things on generate: shows the Mapping rows (as before), AND commits the current `chartDetails`/`tableRowDetails` as the connector's new complete mapping state via `POST /api/setup/mapping`. A failure in the second step is surfaced but doesn't block the first — the person can still paste the generated rows manually even if the "remember this for next time" save failed.
- Step 2's copy changed from "paste new rows at the bottom" to "paste at A1, replacing everything" — this is a real behavior change now that the wizard tracks and can reload full state, not just append blindly.
**Breaking:** Yes, in the sense that a Mapping tab is now expected to be fully regenerated by the wizard each time, not hand-edited. A manual edit made directly in a Mapping tab, without a follow-up "Edit mapping" wizard visit to record it, risks being silently overwritten the next time someone does use the wizard for that connector — this is the explicit tradeoff of introducing a second source of truth (the repo JSON), accepted deliberately in the design discussion rather than an oversight.
